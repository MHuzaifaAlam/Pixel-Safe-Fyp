...
class VisualOverlay:
    ...
    def create_tamper_overlay(original_path, suspicious_array, output_path):
        ...
        try:
            ...
            # Vectorized difference calculation (much faster than loops)
            if len(orig_float.shape) == 3:  # RGB image
                ...
                diff = (diff_r + diff_g + diff_b) / 3.0  # Average across channels
            else:  # Grayscale
            ...
            return {
                ...
                'statistics': stats,
                'color_legend': {
                    'dark_red': 'Heavy tampering (>70% change)',
                    'bright_red': 'Medium tampering (40-70% change)',
                    'orange': 'Light tampering (20-40% change)',
                    'yellow': 'Very light tampering (5-20% change)',
                    'no_color': 'No tampering (<5% change)'
                }
            }
